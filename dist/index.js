...
const typecheck = (projectPath) => {
    ...
    const errThreshold = Number((0, core_1.getInput)("error_fail_threshold") || 0);
    const logString = `Found ${errors} errors!`;
    console.log(logString);
    if (errors > errThreshold) {
        (0, core_1.setFailed)(logString);
    }
};
const performIncrementalCompilation = (ts, projectPath) => {
    const report = (0, reporter_1.reporter)(ts);
...
const loadTS = (projectPath) => {
    try {
        ...
        throw Object.assign(new Error(`Failed to find project specific typescript at ${projectPath}`), { cause: error });
    }
};
main();
...
const reporter = (ts) => (diagnostic) => {
    switch (diagnostic.category) {
        case ts.DiagnosticCategory.Error: {
            return (0, command_1.issueCommand)('error', (0, exports.readProperties)(diagnostic), ts.flattenDiagnosticMessageText(diagnostic.messageText, '\n'));
        }
        case ts.DiagnosticCategory.Warning: {
            return (0, command_1.issueCommand)('warning', (0, exports.readProperties)(diagnostic), ts.flattenDiagnosticMessageText(diagnostic.messageText, '\n'));
        }
    }
};
exports.reporter = reporter;
const readProperties = ({ start, file }) => {
    ...
    const { line, column } = (0, exports.parseLocation)(content, start);
    return { file: fileName, line: `${line}`, col: `${column}` };
};
exports.readProperties = readProperties;
const parseLocation = (content, position) => {
    let l = 1;
    let c = 0;
